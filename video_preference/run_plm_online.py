...
def online_test(args, pipeline, models_dir):
    ...
    # --- 2. 加载模型 ---
    file_prefix = f'epochs_{args.epochs}_lr_{args.lr}_rank_{args.rank}_ts_1755762578.253749'
    best_model_path = os.path.join(models_dir, file_prefix, 'best_model')
    model_path = args.model_path if args.model_path is not None else best_model_path
            ...
            for row in reader:
                ...
                for k in feature_keys:
                    try:
                        ...
                        parsed_sample[k] = ast.literal_eval(row[k])
                    except (ValueError, SyntaxError):
                        # 如果解析失败，则存储空列表
                        parsed_sample[k] = []
                
                video_time_index[key] = parsed_sample
                ...
                with torch.no_grad():
                    ...
                    for offset in reversed(range(args.his_window)):
                        ...
                        for j, k in enumerate(feature_keys):
                            # 获取预加载的列表
                            feature_list = past_sample.get(k)
                            # 填充到矩阵中，如果列表长度小于30，则只填充可用的部分
                            num_frames = min(30, len(feature_list))
                            if num_frames > 0:
                                per_frame_features[:num_frames, j] = feature_list[:num_frames]
                    ...
                    prob_class1 = pred.squeeze(-1)
                    probabilities = torch.stack((1 - prob_class1, prob_class1), dim=-1)
                    prob_list = probabilities.squeeze(0).tolist()
                    ...
                    predicted_labels = (prob_class1 >= 0.5).long().squeeze(0).tolist()
